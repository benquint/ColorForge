...
class SAM2: ObservableObject {
    ...
    @Published var imageEncodings: SAM2LargeImageEncoderFLOAT16Output?
    @Published var promptEncodings: SAM2LargePromptEncoderFLOAT16Output?

    @Published private(set) var initializationTime: TimeInterval?
    @Published private(set) var initialized: Bool?

    private var imageEncoderModel: SAM2LargeImageEncoderFLOAT16?
    private var promptEncoderModel: SAM2LargePromptEncoderFLOAT16?
    private var maskDecoderModel: SAM2LargeMaskDecoderFLOAT16?
    ...
    private func loadModels() async {
        ...
        do {
            ...
            let (imageEncoder, promptEncoder, maskDecoder) = try await Task.detached(priority: .userInitiated) {
                let imageEncoder = try SAM2LargeImageEncoderFLOAT16(configuration: configuration)
                let promptEncoder = try SAM2LargePromptEncoderFLOAT16(configuration: configuration)
                let maskDecoder = try SAM2LargeMaskDecoderFLOAT16(configuration: configuration)
    ...
    func getImageEncoding(from url: URL) async throws {
        ...
        let inputs = try SAM2LargeImageEncoderFLOAT16Input(imageAt: url)
