...
struct SAMSegmentation: Hashable, Identifiable {
    ...
    var mask: CIImage = CIImage.black
    
    var title: String = ""
    var firstAppearance: Int?
    var isHidden: Bool = false
    
    var tintedImage: CIImage?
    ...
    private mutating func updateMask() {
        let ciColor = CIColor(color: .white)
        let monochromeFilter = CIFilter.colorMonochrome()
        monochromeFilter.inputImage = image
        monochromeFilter.color = ciColor!
        monochromeFilter.intensity = 1.0
        guard  let whiteImage = monochromeFilter.outputImage else {return}
        
        print("Image Extent = \(image.extent)")
        
        mask = whiteImage.composited(over: mask).cropped(to: image.extent)
        
//        debugSave(mask, "DebugMask")
    }
