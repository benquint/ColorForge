...
extension CIImage {
	...
	func resizeAndRenderToBuffer() -> CVPixelBuffer? {
		let xScale = 1024.0 / self.extent.width
		let yScale = 1024.0 / self.extent.height
		
		let scaled = self.transformed(by: CGAffineTransform(scaleX: xScale, y: yScale))
		
		let context = RenderingManager.shared.mainImageContext
		
		let buffer = context.render(scaled, pixelFormat: kCVPixelFormatType_32ARGB)
