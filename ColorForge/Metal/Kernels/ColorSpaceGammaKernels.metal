...
//		) * rgb;
	...
	float4 encodeSensor(coreimage::sample_t s, float baselineEV) {
		...
		float3 logPart = ei400_C * log10(ei400_A * linearRGB + ei400_B) + ei400_D;
		float3 linPart = ei400_E * linearRGB + ei400_F;

		// Mask: 1 where x > cut, 0 otherwise
		float3 mask = step(ei400_Cut, linearRGB);
