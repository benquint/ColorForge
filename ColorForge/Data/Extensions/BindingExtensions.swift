...
extension DataModel {
    ...
    // Image
    func bindingToAiMaskMaskImage<T>(
        maskId: UUID,
        keyPath: WritableKeyPath<AiMask, T>,
        defaultValue: T
    ) -> Binding<T> {
        Binding<T>(
            get: {
                ...
                let allMaskIDs = self.items[imageIndex].maskSettings.aiMasks.map { $0.id }

                guard let maskIndex = self.items[imageIndex].maskSettings.aiMasks.firstIndex(where: { $0.id == maskId }) else {
                    return defaultValue
                }

                return self.items[imageIndex].maskSettings.aiMasks[maskIndex][keyPath: keyPath]
            },
            set: { newValue in
                let viewModel = ImageViewModel.shared
                guard
                    let id = viewModel.currentImgID,
                    let imageIndex = self.itemIndexMap[id],
                    let maskIndex = self.items[imageIndex].maskSettings.aiMasks.firstIndex(where: { $0.id == maskId })
                else {
                    return
                }

                var item = self.items[imageIndex]
                var mask = item.maskSettings.aiMasks[maskIndex]

                mask[keyPath: keyPath] = newValue
                item.maskSettings.aiMasks[maskIndex] = mask
                self.items[imageIndex] = item
                ...
                self.undoManager?.registerUndo(withTarget: self) { targetSelf in
                    guard let undoIndex = targetSelf.itemIndexMap[id] else { return }
                    var undoItem = targetSelf.items[undoIndex]
                    var undoMask = undoItem.maskSettings.aiMasks[maskIndex]
                    undoMask[keyPath: keyPath] = mask[keyPath: keyPath]
                    undoItem.maskSettings.aiMasks[maskIndex] = undoMask
                    targetSelf.items[undoIndex] = undoItem
                    FilterPipeline.shared.applyPipelineV2Sync(id, targetSelf)
                }
    ...
    func bindingToAiMaskValue<T: Equatable>(
        maskId: UUID,
        keyPath: WritableKeyPath<AiMask, T>,
        defaultValue: T
    ) -> Binding<T> {
