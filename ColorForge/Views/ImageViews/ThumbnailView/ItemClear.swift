...
struct ItemClear: View {
	...
	var body: some View {
		...
		ZStack {
			...
			ZStack {
				...
				if let cgImage = item.thumbnailImage {
					let nsImage = NSImage(
						cgImage: cgImage,
	...
	private func updateItem() {
		if viewModel.currentImgID == item.id {
			...
			Task (priority: .userInitiated) {
				...
				guard let cgImage = await self.convertCIImage_CG(scaled) else {return}
	...
	// MUCH QUICKER
	private func convertCIImage_CG(_ input: CIImage) async -> CGImage? {
