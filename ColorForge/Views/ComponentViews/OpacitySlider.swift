    ...
    var body: some View {
        HStack {
            ...
            let width: CGFloat = 100
                     ...
                     .gesture(
                         DragGesture(minimumDistance: 0)
                             .onChanged { gesture in
                                 ...
                                 $opacity.wrappedValue = min(max(stepped, 0), 100)
                                 dragValue = clampedX
                                 thumbPos = clampedX
                             }
                             .onEnded { _ in
             ...
             .onAppear {
                 thumbPos = position(for: $opacity.wrappedValue, in: width)
             }
             .onChange(of: $opacity.wrappedValue) { newVal in
                 if dragValue == nil {
            ...
            TextField("", value: $opacity, formatter: wholeNumber)
                .id($opacity.wrappedValue) // Forces redraw when value changes
