...
extern "C" {
	...
	// Exposure
	float4 exposure(coreimage::sample_t s, float exposure) {
		float3 rgb = float3(s.r, s.g, s.b);
		rgb = rgb * exp2(exposure);
		return float4(rgb, s.a);
	}
	
	// Contrast new (curve based)
	// Expects image in log C
	float4 contrast(coreimage::sample_t s, float contrast) {
	...
	// Saturation (needs image in spherical coord e.g sph)
	float4 globalSaturation(coreimage::sample_t s, float saturation) {
	...
	float4 HDRKernel(coreimage::sample_t s, float hdrWhite, float hdrHighlight, float hdrShadow, float hdrBlack) {
		float3 logC = float3(s.r, s.g, s.b);
	...
	/*
	 ...
	 5 = Yellow
	 */
	float4 previewHueRange(coreimage::sample_t s, int colorChoice)
	{
		float3 rgb = float3(s.r, s.g, s.b);
		...
		else if (colorChoice == 1) { weight = bellCurveLooping(hueChannel, g_pos, g_rad);}
		else if (colorChoice == 2) { weight = bellCurveLooping(hueChannel, b_pos, b_rad);}
		else if (colorChoice == 3) { weight = bellCurveLooping(hueChannel, c_pos, c_rad);}
		else if (colorChoice == 4) { weight = bellCurveLooping(hueChannel, m_pos, m_rad);}
		else if (colorChoice == 5) { weight = bellCurveLooping(hueChannel, y_pos, y_rad);}
		
		float4 transparency = float4(0.0, 0.0, 0.0, 0.0);
		float4 maskColor = float4(1.0, 1.0, 1.0, 1.0); // Midtone for log c
		float4 mask = mix(maskColor, transparency, weight);
		
		return mask;
	}
	
	float4 hsdKernel(coreimage::sample_t s,
